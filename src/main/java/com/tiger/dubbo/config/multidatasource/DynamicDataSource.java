...
/**
 * 功能：获取动态数据源
 * 配置时，targetDataSources必须配置一个名为master的数据源，不能配置名为replica-set-slave-999999-host的数据源
 * @author Tiger
 *
 */
...
public class DynamicDataSource extends AbstractRoutingDataSource {
	...
	protected Object determineCurrentLookupKey() {
        ...
        if (Constants.SLAVE_DATASOURCE_CODE.equals(key)) {	//从机代号，则在从机集群中选出一个
        	...
        	DynamicDataSourceHolder.putDataSource(key==null?Constants.MASTER_DATASOURCE:key+"");	//缓存从第一个会话中的数据源
        }
