...
public class DubboBaseConfig {
	...
	public ProtocolConfig protocol() {
		...
		protocolConfig.setPort(Integer.parseInt(Constants.appConfig.getProperty("dubbo.protocol.port").trim())); // dubbo.protocol.port
		...
		if ("fixed".equalsIgnoreCase(threadpool)) // 固定线程池时设置线程池大小
			protocolConfig
					.setThreads(Integer.parseInt(Constants.appConfig.getProperty("dubbo.protocol.threads").trim())); // dubbo.protocol.threads
																														// //服务线程池大小(固定大小)
		String iothreads = Constants.appConfig.getProperty("dubbo.protocol.iothreads");
		if (iothreads != null && !"".equalsIgnoreCase(iothreads.trim()))
			protocolConfig.setIothreads(Integer.parseInt(iothreads)); // dubbo.protocol.iothreads
																		// //io线程池大小(固定大小),默认为cpu个数+1

		protocolConfig.setBuffer(Integer.parseInt(Constants.appConfig.getProperty("dubbo.protocol.buffer").trim())); // dubbo.protocol.buffer
																														// //网络读写缓冲区大小
		protocolConfig
				.setHeartbeat(Integer.parseInt(Constants.appConfig.getProperty("dubbo.protocol.heartbeat").trim())); // dubbo.protocol.heartbeat
